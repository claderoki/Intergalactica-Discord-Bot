...
class ColorHelper:
    @classmethod
    def calculate_dominant_color(cls, url):
        color_thief = ColorThief(requests.get(url, stream=True).raw)
        dominant_color = color_thief.get_color(quality=1)
        return discord.Color.from_rgb(*dominant_color)

    @classmethod
    def get_primary_color(cls) -> discord.Colour:
    ...
    def get_random_color(cls, start_range=80, end_range=255) -> discord.Colour:
        # from 80 to 255 because the majority of disc users use the dark theme, and anything under 80 is too bright to be comfortably visible.
        if end_range > 255:
            end_range = 255
