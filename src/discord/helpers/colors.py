...
class ColorHelper:
    def __get_dominant_color(url):
        color_thief = ColorThief(requests.get(url, stream=True).raw)
        dominant_color = color_thief.get_color(quality=1)
        return discord.Color.from_rgb(*dominant_color)

    def get_dominant_color(self, guild = None) -> discord.Colour:
        return discord.Color.from_rgb(242, 180, 37)

    def get_random_color(self, start_range=80, end_range=255) -> discord.Colour:
