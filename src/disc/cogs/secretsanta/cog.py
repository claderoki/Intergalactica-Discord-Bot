...
from src.disc.cogs.intergalactica import Intergalactica
...
from src.disc.helpers.waiters.base import DateWaiter, TimeWaiter
...
class SecretSantaCog(BaseCog, name="Secret Santa"):
    ...
    async def secret_santa_create(self, ctx):
        ...
        await participant.editor_for(ctx, "type", properties_to_skip=[x for x in SecretSantaParticipant.Type if
                                                                      x not in available_types])
    ...
    for type in participants:
        ...
        for participant in participants[type]:
            for _ in range(3):
                ...
                if giftee.user_id not in participant_mapping.get(giftee.user_id,
                                                                 []) and giftee.user_id not in participant_mapping.get(
                        participant.user_id, []):
                    break
